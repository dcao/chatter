defmodule ChatterWeb.AdminAuth do
  ...
  def log_out_admin(conn) do
    ...
    admin_token && Orgs.delete_admin_session_token(admin_token)

    if live_socket_id = get_session(conn, :live_socket_id) do
      ChatterWeb.Endpoint.broadcast(live_socket_id, "disconnect", %{})
    end
  ...
  def fetch_current_admin(conn, _opts) do
    ...
    admin = admin_token && Orgs.get_admin_by_session_token(admin_token)
  ...
  the current_admin:
      ...
      defmodule ChatterWeb.PageLive do
        ...
        on_mount {ChatterWeb.AdminAuth, :mount_current_admin}
        ...
      end

  Or use the `live_session` of your router to invoke the on_mount callback:

      live_session :authenticated, on_mount: [{ChatterWeb.AdminAuth, :ensure_authenticated}] do
        live "/profile", ProfileLive, :index
      end
  """
  def on_mount(:mount_current_admin, _params, session, socket) do
    {:cont, mount_current_admin(session, socket)}
  end

  def on_mount(:ensure_authenticated, _params, session, socket) do
    socket = mount_current_admin(session, socket)
  ...
  defp mount_current_admin(session, socket) do
    case session do
      %{"admin_token" => admin_token} ->
        Phoenix.Component.assign_new(socket, :current_admin, fn ->
          Orgs.get_admin_by_session_token(admin_token)
        end)

      %{} ->
        Phoenix.Component.assign_new(socket, :current_admin, fn -> nil end)
    end
  end
