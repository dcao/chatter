defmodule ChatterWeb.AdminAuthTest do
  use ChatterWeb.ConnCase, async: true

  alias Phoenix.LiveView
  alias Chatter.Orgs
  alias ChatterWeb.AdminAuth
  import Chatter.OrgsFixtures

  @remember_me_cookie "_chatter_web_admin_remember_me"

  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, ChatterWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "log_in_admin/3" do
    test "stores the admin token in the session", %{conn: conn, admin: admin} do
      ...
      assert Orgs.get_admin_by_session_token(token)
    end
  ...
  describe "logout_admin/1" do
    test "erases session and cookies", %{conn: conn, admin: admin} do
      admin_token = Orgs.generate_admin_session_token(admin)
      ...
      assert redirected_to(conn) == ~p"/"
      refute Orgs.get_admin_by_session_token(admin_token)
    end

    test "broadcasts to the given live_socket_id", %{conn: conn} do
      live_socket_id = "admins_sessions:abcdef-token"
      ChatterWeb.Endpoint.subscribe(live_socket_id)
  ...
  describe "on_mount: mount_current_admin" do
    ...
    test "assigns nil to current_ admin assign if there isn't a valid admin_token ", %{conn: conn} do
    ...
    test "assigns nil to current_ admin assign if there isn't a admin_token", %{conn: conn} do
  ...
  describe "on_mount: :redirect_if_admin_is_authenticated" do
    ...
    test "Don't redirect is there is no authenticated admin", %{conn: conn} do
      session = conn |> get_session()
