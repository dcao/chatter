defmodule ChatterWeb.AdminConfirmationInstructionsLiveTest do
  ...
  describe "Resend confirmation" do
    ...
    test "sends a new confirmation token", %{conn: conn, admin: admin} do
      ...
      assert Repo.get_by!(Orgs.AdminToken, admin_id: admin.id).context == "confirm"
    end

    test "does not send confirmation token if admin is confirmed", %{conn: conn, admin: admin} do
      Repo.update!(Orgs.Admin.confirm_changeset(admin))
      ...
      refute Repo.get_by(Orgs.AdminToken, admin_id: admin.id)
    end
